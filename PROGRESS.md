...
### 伤害积分召唤支援系统（F5）✅
- **做了什么**：用你打出的**伤害当积分**，按 **F5** 打开支援菜单，进小地图**长按锁定落点**召唤打击。
  - **伤害积分**：`earnDamage` 累计你对生物造成的伤害（枪/近战/火箭/坦克炮爆炸/震撼弹都计入）；HUD 左下显示。
  - **F5 菜单**：4 张卡（名字/说明/花费），伤害积分够才可选；选中→进以玩家为中心的小地图，光标处显示 AOE 预览圈，**长按 0.6s 锁定**（按真实时间，任何帧率都一样）→ 扣积分、召唤打击、关闭。打开时冻结模拟（不算暂停）。
  - **四种打击**（都从高空落下 + 地面预警圈）：**炮兵齐射(800)** 18 发炮弹覆盖目标区；**制导导弹(1000)** 落地 30m 内生物立即死亡；**核弹(2000)** 慢慢降下、45m 内立即死亡 + 白屏闪光；**冷冻弹药(1200)** 30m 内全部冻住（除你）。
- **改了哪些文件**：`config/gameplay.js`(支援配置)、`main.js`(伤害积分/earnDamage 埋点、打击体/齐射/区域效果 killArea/freezeArea/areaDamage、F5 菜单+小地图长按 UI、冻结模拟、HUD、flashScreen、重开清理)、`abilities.js`(震撼弹伤害计入积分)、`index.html`(支援界面 DOM/CSS、伤害积分 HUD、白屏闪光)。
- **自测**：伤害累计(震撼弹4命中+180)；制导/核弹半径内 8/8 秒杀；冷冻 8/8 冻住；炮兵齐射 18 发排队；F5 菜单 4 卡按积分启用/禁用；选中进小地图 AOE 预览；长按逻辑(holding→hold 累计→锁定)正常；小镇 selftest 仍 ok；0 报错。截图 `callin-menu.png`/`callin-map.png`。
- **给你早上的问题**：四种的花费/半径/威力都在 `gameplay.js` 的 `支援`；伤害积分怎么算的在 `main.js` 的 `earnDamage` 埋点处。
